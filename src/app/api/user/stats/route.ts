...
export async function GET(_request: NextRequest) {
  try {
    ...
    const validMovieDetails = movieDetails.filter(
      (movie: any) => movie !== null,
    );

    // Вычисляем статистику
    const totalWatched = watchedMovies.length;
    const totalWatchTime = validMovieDetails.reduce(
    ...
    const ratingsCount = watchedMovies.filter(
      (movie: any) => movie.rating,
    ).length;
    ...
    watchedMovies.forEach((movie: any) => {
      if (movie.watched_at) {
        ...
        const monthKey = watchedDate.toLocaleDateString("ru-RU", {
          year: "numeric",
          month: "long",
        });
        if (Object.hasOwn(monthlyCount, monthKey)) {
