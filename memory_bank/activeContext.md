...
## Текущий фокус
Работа над функциональными страницами. Таблица `user_movies` создана, база данных работает. Текущий этап: стабилизация полного пользовательского флоу и синхронизация проектной документации. Прогресс: ~60%

Текущая задача в работе: синхронизировать `memory_bank` по актуальному `AGENTS.md`, привести `## Project Deliverables` к каноническому табличному формату и зафиксировать изменения через git commit/push.
...
## Последние изменения
- 2026-04-28: `memory_bank/projectbrief.md` приведен к каноническому формату `## Project Deliverables` из актуального `AGENTS.md`: таблица переведена на колонки `ID | Deliverable | Status | Weight`, сохранены канонические статусы, веса нормализованы как целые числа с суммой 100
- 2026-04-28: Исправлена консистентность `user_movies` в API `favorite` / `watched` / `rating`: частичные действия пользователя больше не затирают соседние поля (`is_favorite`, `is_watched`, `rating`, `watched_at`) при `upsert`
- 2026-04-28: Добавлен `DELETE /api/user/movies/[id]/watched`; удаление из списка просмотренных теперь соответствует поведению UI страницы `/watched`
- 2026-04-28: Во время верификации выявлены внешние blockers окружения: legacy-копия проекта в `popflix/` попадает в `tsc` include и создает шум в типах; `bunx next build` падает на определении workspace root/Turbopack несмотря на `turbopack.root`
...
1. Проверить в браузере полный user flow после серверных фиксов `favorite/watched/rating`
2. Убрать влияние legacy-папки `popflix/` на типизацию и проверки проекта
3. Разобраться с падением `next build` / Turbopack root resolution в текущем окружении
...
- Папка `popflix/` содержит legacy-копию приложения и сейчас попадает под `tsconfig include`, из-за чего `bunx tsc --noEmit` шумит тысячами нерелевантных ошибок
- В текущем окружении `bun run build` не резолвит локальный `next`, а `bunx next build` падает на определении workspace root для Turbopack
